...
namespace UsingsRU.Patterns
{
    ...
    public class Interval<T> : IEquatable<Interval<T>> where T : IComparable<T>
    {
        ...
        {
            return (min.CompareTo(value.Min) > 0 ? min : value.Min).CompareTo(max.CompareTo(value.Max) < 0 ? max : value.Max) <= 0;
        }
        ...
        {
            List<Interval<T>> tmp = new List<Interval<T>>();
        ...
        {
            return IsCombination(value) ? new Interval<T>(min.CompareTo(value.Min) < 0 ? min : value.Min, max.CompareTo(value.Max) > 0 ? max : value.Max) : null;
        }
        ...
        public List<Interval<T>> Combine(List<Interval<T>> values)
        {
            ...
            Interval<T> cur = this;
            List<Interval<T>> curValues = new List<Interval<T>>(values.Count);
